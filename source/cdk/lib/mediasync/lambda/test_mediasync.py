...
class TestMediaSyncLambdaFunction(unittest.TestCase):
    ...
    def test_check_if_deleted_success(self):
        with mock.patch.dict(os.environ, {'LogLevel': 'INFO', 'SOLUTION_IDENTIFIER': 'AwsSolution/SO0133/1.1.0'}):
    ...
    def test_submit_job_success(self):
        with mock.patch.dict(os.environ, {'JOB_DEFINITION': self.job_definition_arn,'JOB_DEFINITION_X_REGION': self.job_definition_arn, 'JOB_QUEUE': self.job_q_arn, 'DESTINATION_BUCKET_NAME': DESTINATION_S3_BUCKET_NAME, 'DISABLE_PENDING_JOBS_CHECK': "true", 'MAX_NUMBER_OF_PENDING_JOBS': "96", 'MN_SIZE_FOR_BATCH_IN_BYTES': "524288000", 'LogLevel': 'INFO', 'SOLUTION_IDENTIFIER': 'AwsSolution/SO0133/1.1.0'}):
    ...
    def test_is_can_submit_jobs_error(self):
        with mock.patch.dict(os.environ, {'JOB_DEFINITION': self.job_definition_arn,'JOB_DEFINITION_X_REGION': self.job_definition_arn, 'JOB_QUEUE': 'self.job_q_arn', 'DESTINATION_BUCKET_NAME': DESTINATION_S3_BUCKET_NAME, 'DISABLE_PENDING_JOBS_CHECK': 'False', 'MAX_NUMBER_OF_PENDING_JOBS': "-1", 'MN_SIZE_FOR_BATCH_IN_BYTES': "524288000", 'LogLevel': 'INFO', 'SOLUTION_IDENTIFIER': 'AwsSolution/SO0133/1.1.0'}):
            from mediasync_driver.app import is_can_submit_jobs
