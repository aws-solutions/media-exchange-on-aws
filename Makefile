...
ifneq ($(SUBSCRIBER_ACCOUNT_ID), $(ACCOUNT_ID))
	...
	@aws cloudformation describe-stacks --stack-name $(STACKPREFIX)-agreement-$(ENV) --query "Stacks[0].Outputs[?OutputKey == 'PublisherOnboardingSummary'].OutputValue" --output text > $(CURRENT_DIR)/tests/publisher.env

	@echo saving subscriber onboarding info at tests/subscriber.env
	@aws cloudformation describe-stacks --stack-name $(STACKPREFIX)-agreement-$(ENV) --query "Stacks[0].Outputs[?OutputKey == 'SubscriberOnboardingSummary'].OutputValue" --output text > $(CURRENT_DIR)/tests/subscriber.env
