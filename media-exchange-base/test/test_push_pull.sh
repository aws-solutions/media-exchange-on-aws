...
##overriding these to map to the testrole names when root roles are used. 
PUBLISHER_ROLE_F=$(echo $PUBLISHER_ROLE | sed 's/:root/:role\/publisher-role/g')
SUBSCRIBER_ROLE_F=$(echo $SUBSCRIBER_ROLE | sed 's/:root/:role\/subscriber-role/g')
...
resp=$(aws sts assume-role --role-arn $PUBLISHER_ROLE_F --role-session-name $SESSION_NAME)
...
echo copying to s3
aws s3api put-object --bucket $BUCKET_NAME --key $FILE_NAME --body /tmp/$FILE_NAME --server-side-encryption aws:kms --ssekms-key-id $KMS_KEY_ARN --grant-read id=$SUBSCRIBER_CANONICAL_USER_ID > /dev/null
...
resp=$(aws sts assume-role --role-arn $SUBSCRIBER_ROLE_F --role-session-name $SESSION_NAME)
