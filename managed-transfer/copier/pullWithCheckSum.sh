...
aws s3 cp $1 $2 --copy-props metadata-directive --metadata-directive REPLACE --metadata Content-MD5=$(cat /tmp/MD5.result),Content-SHA1=$(cat /tmp/SHA1.result),Content-SHA256=$(cat /tmp/SHA256.result),Content-SHA512=$(cat /tmp/SHA512.result)
